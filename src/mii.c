...
/* 32-bit ARM doesn't support __int128, use uint64_t (limits RAMWORKS to 64 banks) */
#if defined(__SIZEOF_INT128__)
typedef unsigned __int128 mii_ramworks_bits_t;
#else
typedef uint64_t mii_ramworks_bits_t;
#endif
#include "mii_video.h"
#include "mii_sw.h"
#include "mii_65c02.h"
...
{
	if (bank > 127 ||
			!(mii->ramworks.avail & ((mii_ramworks_bits_t)1ULL << bank)))
		bank = 0;
	if (!mii->ramworks.bank[bank]) {
		...
		for (int i = 0; i < 128; i++ ) {
			if (mii->ramworks.bank[i])
				c++;
			if (mii->ramworks.avail & ((mii_ramworks_bits_t)1ULL << i))
				a++;
		}
...
#define B(x) ((mii_ramworks_bits_t)1ULL << (x))
static const mii_ramworks_bits_t _mii_ramworks3_config[] = {
